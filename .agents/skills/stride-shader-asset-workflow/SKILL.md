...
## Built-in Shaders And Inheritance Levels (For Creation/Modification)

Only use this section to choose where a new shader should inherit from.
This list is extracted from Stride built-in shader sources under:
- `sources/engine/Stride.Graphics/Shaders`
- `sources/engine/Stride.Rendering/Rendering`
- `sources/engine/Stride.Particles/Shaders`

### 1) Global foundation (top-level)

- `ShaderBaseStream` (root rendering stream container)
- `ShaderBase : ShaderBaseStream`
- `Texturing` (texture/sampler utility root)
- `Camera` (camera/depth utility root)

Recommended:
- Most renderable shaders should indirectly include `ShaderBase`.
- Any shader sampling textures should include/inherit `Texturing`.

### 2) 2D / PostFX chain

- `SpriteBase : ShaderBase, Texturing`
- `ImageEffectShader : SpriteBase`
- `BackgroundShader : SpriteBase`
- `BackgroundCubemapShader : BackgroundShader`

Typical extension points:
- Custom sprite-like pass: inherit `SpriteBase`
- Fullscreen image post-process: inherit `ImageEffectShader`
- Sky/background variants: inherit `BackgroundShader`

### 3) Core shading chain (3D forward/deferred common)

- `ShadingBase : ShaderBase`
- `MaterialSurfaceStreamShading : ShadingBase, MaterialPixelShadingStream`
- `MaterialSurfacePixelStageCompositor : ShadingBase, Transformation, PositionStream, MaterialPixelShadingStream, ...`

Typical extension points:
- Need custom `PSMain` shading output path: start from `ShadingBase`
- Material surface final lighting composition: use `MaterialSurfacePixelStageCompositor` family

### 4) Material stream/interface hierarchy

- `IStreamInitializer` (root interface-style shader)
- `MaterialStream : IStreamInitializer`
- `MaterialPixelStream : MaterialStream, NormalStream, LightStream`
- `MaterialPixelShadingStream : MaterialPixelStream, LightStream`
- `MaterialVertexStream : MaterialStream, MaterialDisplacementStream, NormalStream, PositionStream`
- `MaterialDomainStream : MaterialStream, MaterialDisplacementStream, MaterialTessellationStream, NormalStream, PositionStream, Texturing`

Typical extension points:
- Add new per-material data field: extend `MaterialStream` descendants (`MaterialPixelStream` / `MaterialVertexStream` / `MaterialDomainStream`)
- Add pixel-stage shading input/output data: extend `MaterialPixelShadingStream`

### 5) Particle chain

- `DepthBase : Camera, Texturing`
- `ParticleBase : DepthBase, ShaderBase, Texturing, ParticleUtilities`

Typical extension points:
- New particle render shader: inherit `ParticleBase`
- Soft-particle/depth-related behavior: keep `DepthBase` path intact

### 6) Graphics built-ins quick map

- `SignedDistanceFieldFont : Texturing`
- `SignedDistanceFieldFontShader : ShaderBase, SignedDistanceFieldFont`
- `SpriteBatchShader<T> : SpriteBase`
- `SpriteAlphaCutoff<T> : SpriteBase`
- `SpriteEffect : SpriteBase`
- `UIEffectShader<T> : ShaderBase, Texturing`

### 7) Selection rule (important)

- If you are building a fullscreen/post process pass, prefer:
  - `ImageEffectShader`
- If you are building material lighting/surface logic, prefer:
  - `MaterialSurfacePixelStageCompositor` (or related material surface shaders)
- If you are building particles, prefer:
  - `ParticleBase`
- If you are unsure, start from:
  - `ShaderBase` (minimal, safe baseline)
