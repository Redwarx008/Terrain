...
| 河流 surface 后段 map lighting/fog 链 | 🚧 进行中 | `Terrain.Editor/Effects/RiverSurface.sdsl`, `Terrain.Editor/Rendering/River/RiverRenderFeature.cs`, `game/map/water/shadow_color.dds` | 2026-06-19 follow-up：RenderDoc MCP 热修改确认完整 CK3 surface final 与 `CalcWater` see-through 之间的主要暗化来自 wrapper 后段，而不是 FOW：CK3 bank 像素 `FogOfWarAlpha` 采样为 `[1,1,0,1]`，但 `CalcWater` base see-through 约 `[0.098,0.095,0.072]`，完整输出为 `[0.022,0.028,0.030]`；current 同类像素 see-through 直出约 `[0.292,0.212,0.143]`，强制输出 CK3 shadow-tint 目标色 `[0.023,0.023,0.033]` 后落入参考能量范围。`ApplySurfacePostProcessing` 因此重新接回 cloud shadow、terrain shadow tint 和 map distance fog 的 RGB 后段；战争迷雾属于 strategy 层，`FogOfWarAlphaTexture` / sampler / `_HasFogOfWarAlphaTexture` / `SurfaceFogOfWarAlphaTexture` 仍保持删除状态。Runtime streaming terrain height provider 仍待单独接入。 |
...
| 河流 surface alpha / water-color 导入 | ✅ 已修正 | `Terrain.Editor/Effects/RiverSurface.sdsl`, `game/map/water/water_color.dds`, `Terrain.Editor.Tests/RiverShaderTextTests.cs` | 2026-06-19 更新后的 `debug.rdc` 复核确认 foam 白块已消失，剩余浅岸发黑来自 surface alpha 不等价：current 用 `_BankFade` edge fade 导致 bank-edge alpha 仍为 `1.0`，CK3 surface 使用 `saturate(Depth * 2.0 / _Depth) * Transparency * connectionFade`。RenderDoc MCP 热替换在 `(1000,620)` 证明目标 alpha 约 `0.303` 会让 post-blend 从暗 surface shaderOut 回到亮场景混合结果；`RiverSurface` 已改为 depth-based alpha，并用文本测试禁止回退到底部 pass 的 bank fade alpha。同日 `debug3.rdc` 复核确认 Stride sRGB 内容导入路径会让 `WaterColorTexture` 比 CK3 `BC7_SRGB` 更暗；因此 `water_color.dds` 作为本地 DDS 以线性/UNORM 方式通过 `Texture.Load(..., loadAsSrgb:false)` 读取，surface shader 只对 RGB 手动 sRGB decode，alpha/spec 不变。 |
