...
| 路径渲染 — 河流 | ✅ | `Terrain.Editor/Rendering/River/`, `Terrain.Editor/Effects/River*.sdsl`, `Terrain.Editor/Assets/River/` | [ADR-014](log/decisions/adr-014-river-rendering-architecture.md)；河流渲染现在使用 CK3 对齐的 `bottom -> refraction -> surface` 三段链路：双 pass 已绑定 CK3 风格底部 diffuse/normal/properties/depth、水面 flow/ambient normal、water-color、foam/foam ramp/foam map/foam noise 与 reflection/specular 资源；DDS 均有 `.sdtex` 描述进入 Stride 内容库；`RiverRenderFeature` 维护分离的 `SceneSeedColor` 与 `BottomColor`，scene seed 先下采样到前者，再 copy 到 working refraction buffer；bottom shader 当前按 2026-06-17 RenderDoc capture 对齐到 CK3 non-advanced bottom 路径：steep parallax 仍使用 river ribbon/tangent UV 求偏移，但 `BottomDiffuse` / `BottomNormal` / `BottomProperties` 主采样统一改为 parallax 后的 `worldUv`，depth/profile 保持由 `tangentUv` 驱动，alpha 改为 `fadeOut * connectionFade * saturate(depth * 13.0f)`，不再使用 `BottomNormal.b` depth shaping 或 advanced diffuse-alpha bank fade；其中 steep parallax 的最终插值权重现已改成保符号分母 + `saturate`，用来阻止河心像素把 `bottomWorldPosition` 横向外插到错误位置；surface shader 按 CK3 map UV 语义采 water-color：surface 处采样时执行 Y 翻转并使用 alpha 做 gloss/spec，surface map UV 现使用 `float2(width - 1, height - 1)` 分轴归一化而不是复用 `_MapExtent` 标量，refraction/see-through tint 在反解出的 bottom/refraction world position 重新采样 RGB，并通过专用 `WaterColorSampler` 保持 CK3 同语义的 `LinearWrap`，surface alpha 用显式 bank fade，refraction sampler 改为 `PointClamp` 以减少河岸颜色串扰；bottom RT alpha 直接存 camera-relative bottom distance 且不再按 coverage 混合，seed-only 边缘像素的 `a<=0` 会在 surface 端被视为无效 payload 并回退到当前水面位置，而 see-through attenuation 现改为使用 `effectiveDepth = min(surfaceDepth, refractionDepth)`，减少河心因错误底距产生的过暗衰减；bottom pass 按 CK3 lit material 结构使用 direct sun、真实 directional shadow 与 environment cubemap IBL 生成 refraction 底色，不再使用 diffuse multiplier 补偿亮度；`RiverRenderFeature` 现在直接绑定当前 `LightingView` 的可见 directional light/skybox、`LightDirectionalShadowMapRenderer.ShaderData` 的 cascade splits / world-to-shadow / depth bias / offset scale / shadow atlas，以及 scene `LightSkybox` 的 cubemap / intensity / rotation，其中 directional 优先选有真实 shadow map 的候选，skybox 优先选有真实 scene cubemap 的候选；2026-06-18 进一步确认 editor terrain 现在默认参与 shadow caster，shared shadow atlas 会写入真实 terrain 深度，修复了 river bottom scene-driven shadow 过去只读到 clear 后空 atlas 的问题；legacy bottom cubemap 仅在场景 skybox 缺失时才按需 lazy 退回 `Skybox texture``，失败会记忆避免逐帧重试，若仍不可用则最后退到 `reflection-specular`；surface 继续使用 `reflection-specular` 资源，但现在按真实 reflection cubemap 采样水面反射；2026-06-17 RenderDoc 复核确认 current `debug.rdc` 中 bottom draw `184/197` 实际读取 scene skybox 预滤波 cubemap，而 surface draw `226/244` 继续读取 reflection cubemap；在此基础上，bottom 最终 lit color 当前增加了一个经 RenderDoc `lighting_x3` 热替换验证的 `3.0f` 能量增益；`RiverRenderSettings` 对 bottom 侧保留的 river-local 参数现在只承担 tuning multiplier（environment/specular/normal strength），不再承担 sun/shadow fallback 语义；shader 将顶点流归一化半宽还原为 full-width，`_MapExtent` 只用于宽度/深度归一化，默认 `BankFade=0.15f`，水面核心 alpha 不受 water-color alpha 限制；pre-bottom seed payload 与 CK3 仍未完全等价，河岸泄漏问题保留为下一轮定位项 |
