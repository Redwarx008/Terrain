...
| 河流 surface 后段 map lighting/fog/cloud 链 | ✅ debug4 完整恢复 | `Terrain.Editor/Effects/RiverSurface.sdsl`, `Terrain.Editor/Rendering/River/RiverRenderFeature.cs`, `Terrain.Editor/Rendering/River/RiverResourceLoader.cs`, `game/map/water/shadow_color.dds` | 2026-06-19 曾通过 RenderDoc MCP 证明 CK3 wrapper 后段会明显影响 surface final，并在本地接入 terrain shadow tint、procedural cloud tint 与 map distance fog，同时删除 strategy-layer FOW。2026-06-20 `debug3.rdc` / `debug4.rdc` 又证明 procedural cloud shadow 会因 `_GlobalTime` 相位导致河流颜色不稳定；随后短暂把 cloud 固定为 0 以获得确定性对比。用户要求完全恢复到 `debug4.rdc` 后，当前状态已恢复完整 debug4-style `ApplySurfacePostProcessing`：`GetCloudShadowMask`、`_HasCloudShadowEnabled`、`_InverseWorldSize`、`_GlobalTime * 0.01f` 云影移动、`cloudMask * 0.8f` 水色暗化、terrain shadow tint、`ApplyOvercastContrast` 和 map distance fog 均重新存在。`RiverRenderFeature` 会绑定 `_InverseWorldSize` 与 `_HasCloudShadowEnabled`；有 editor terrain 时使用 heightmap world size 覆盖 map fallback。策略层 FOW 仍不接入 river surface。注意：这会按 debug4 重新引入时间相位敏感性，多次切换地形显隐再截图时若 `_GlobalTime` 不同，河流颜色仍可能变化。 |
...
| 河流 surface alpha / water-color 导入 | ✅ 已修正 | `Terrain.Editor/Effects/RiverSurface.sdsl`, `Terrain.Editor/Rendering/River/RiverResourceLoader.cs`, `game/map/water/water_color.dds`, `Terrain.Editor.Tests/RiverShaderTextTests.cs` | 2026-06-19 更新后的 `debug.rdc` 复核确认 foam 白块已消失，剩余浅岸发黑来自 surface alpha 不等价：current 用 `_BankFade` edge fade 导致 bank-edge alpha 仍为 `1.0`，CK3 surface 使用 `saturate(Depth * 2.0 / _Depth) * Transparency * connectionFade`。RenderDoc MCP 热替换在 `(1000,620)` 证明目标 alpha 约 `0.303` 会让 post-blend 从暗 surface shaderOut 回到亮场景混合结果；`RiverSurface` 已改为 depth-based alpha，并用文本测试禁止回退到底部 pass 的 bank fade alpha。bottom/water DDS 均绕过 Stride 内容系统从 `game/map/water` 直接读取；`water_color.dds` 当前按 UNorm/linear view 加载（`loadAsSrgb:false`），surface shader 不做手动 sRGB decode。 |
