...
| 河流 surface depth bias | ✅ 已修正 | `Terrain.Editor/Rendering/River/RiverRenderFeature.cs`, `Terrain.Editor.Tests/RiverRenderFeatureRuntimeTests.cs` | 2026-06-20 `debug.rdc` 与 CK3 `ck3-river.rdc` 对比确认，块状露地形不是整段 river 缺失，也不是 CK3 不写 terrain；当前 surface 和 terrain depth 只差 `1e-5` 量级，水面局部落到 terrain 后面即被 `DepthRead` 拒绝。CK3 surface 不输出 `SV_Depth`，通过后主 depth history 仍保留 terrain 值，但 surface `SV_Position.z` 稳定比 terrain 近约 `0.003`。当前实现保留 surface `DepthStencilStates.DepthRead`，不写主 depth，并将 surface rasterizer 从 bottom 分离出来使用更强负向 depth bias；上一轮按 terrain 重新抬高 `RiverMeshService` 顶点的尝试已撤销。 |
