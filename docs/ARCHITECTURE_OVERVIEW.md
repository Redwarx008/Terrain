...
| `Terrain/Effects/Ocean/OceanSurface.sdsl` | Ocean surface pass；采样 CK3-style water DDS，消费 shared refraction capture 的 RGB 与未过滤 alpha payload，实现 core water refraction/fade/see-through/foam/normal/reflection；flow normal 使用 CK3 FlowMap 方向旋转、`SampleGrad`、`_WaterFlowMapSize * 1.5` 四相位 cubic blend，gradients 按 CK3 源码从旋转前 `NormalCoord` 计算；lighting 复用 `RiverStrideLighting` scene light/skybox |
