...
| `Terrain/Rendering/Water/WaterRefractionCapturePass.cs` | Renderer-owned shared water refraction capture helper；从 scene color + Presenter depth 写半分辨率 refraction texture，供 Ocean surface 与 River bottom/surface chain 共享 |
...
| `Terrain/Effects/Ocean/OceanSurface.sdsl` | Ocean surface pass；采样 CK3-style water DDS，消费 shared refraction capture 的 RGB 与未过滤 alpha payload，实现 core water refraction/fade/see-through/foam/normal/reflection，并复用 `RiverStrideLighting` scene light/skybox |
| `Terrain/Effects/Ocean/OceanVertexStreams.sdsl` | Ocean quad 自定义 UV stream |
| `Terrain/Effects/Water/WaterRefractionCapture.sdsl` | Shared water refraction capture image shader；压缩 scene color，并在 alpha 写 RiverCommon camera-relative world-space distance payload |
