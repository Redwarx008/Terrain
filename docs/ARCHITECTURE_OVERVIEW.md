...
**2026-06-24 Ocean shader/render feature 补充：** Ocean 已新增 `OceanVertexStreams` / `OceanSurface` SDSL 和生成的 shader key 文件。`OceanSurface` 继承 `ShaderBase, TransformationWAndVP, OceanVertexStreams, RiverStrideLighting`，采样 `game/map/water` 的 water color、ambient normal、flowmap、flow normal、foam/foam ramp/map/noise，并复用 `RiverStrideLighting` 的 scene-driven light/shadow/skybox helper；禁止接入 province/border/fog-of-war/flatmap/固定太阳等策略层 token。`OceanSurface` 当前在源码/编译链路实现 CK3 core water 语义：shared refraction capture、`Texture2D.Load` 未过滤 alpha payload、base/offset refraction separation、offset above-water reject、see-through attenuation、see-through shore mask、water fade、三层 ambient normals、CK3 FlowMap 方向旋转 + `SampleGrad` + `_WaterFlowMapSize * 1.5` 四相位 cubic flow normal interpolation、shore-mask foam ramp 半 texel clamp、fresnel/environment reflection；`SampleGrad` gradients 按 CK3 源码从旋转前 `NormalCoord` 计算。正常 ocean 像素 alpha 为 `1.0`。`WaterRefractionCapture` 的 RGB 按 CK3 协议直通 scene color，不做 `color / (1 + color) * 1.5` 压缩，alpha 继续写 camera-relative distance payload；`debug1.rdc` / `debug2.rdc` 复核确认 CK3 捕获的 near-black water color、`_WaterDiffuseMultiplier=0` 和 tiny refraction scale 不适配当前 Stride HDR scene/lighting/tonemap，典型海面像素会把 terrain HDR `[2.87,1.84,1.07]` 覆盖为过暗水色；后一轮亮蓝补偿又暴露出另一个 lighting-model 问题：`RiverStrideComputeLighting` 的 scale 只作用于 environment IBL，direct sun 仍直接吃 scene sun intensity `20`，会把 Ocean diffuse 推成高亮青蓝。当前 Ocean 使用 Ocean-local `ComputeOceanLighting` 包装共享 helper，使 direct diffuse/specular 和 environment diffuse/specular 都受 `_OceanSceneLightingScale=0.18` 约束；近景 diffuse 不再直接混入 water color map，water map 只用于 refraction/far-map 路径；颜色能量使用 CK3-hue、Stride 光照模型校准值：`_OceanRefractionColorScale=0.30`、`_WaterReflectionIntensity=0.25`、`_WaterDiffuseMultiplier=0.20`，材质默认 Shallow/Deep 为 `[0.035,0.09,0.11]` / `[0.006,0.024,0.034]`。shared capture、River 和全局 scene light 仍不变。Ocean gloss 由 water color/spec alpha、`_WaterGlossBase=1.15`、`_WaterGlossScale=0.4` 和 `_OceanRoughness` 共同推导，flow normal scale 对齐 CK3 捕获值 `0.025`；Ocean see-through shore mask 保持 CK3/River 公式 `(_WaterSeeThroughShoreMaskDepth - refractionDepth)`，默认阈值为 `3.0`，避免 `0.0` 阈值把所有正深度岸边透底推回纯 water color；Ocean animation 通过 `_OceanWaveSpeedScale=0.2` 和 `_OceanFlowSpeedScale=0.2` 在 shader 内降低 ambient normal、flow normal 与 foam noise 速度，不改 CPU 全局时间绑定。`OceanRenderFeature` 支持 `OceanRenderObject`，初始化 `DynamicEffectInstance("OceanSurface")`、`OceanResourceLoader`、`WaterSceneLightingBinder` 和 single pass pipeline，在 `Prepare` 中绑定 `OceanMaterialSettings.ShallowColor/DeepColor/Roughness/WaveScale` 及 sea level/map size；stage `Draw(...)` no-op 防 selector 残留 double draw，实际 `DrawWater(...)` 由 `CustomForwardRenderer` 调用并绑定 shared refraction、与 capture 相同的 `RiverCommonKeys._RefractionMaxCameraHeight`、FlowMap 实际 `ViewWidth/ViewHeight`、camera/global time/flow time/view/world 矩阵和 scene lighting。`MainScene.sdscene` 现在包含独立 Ocean entity，`GraphicsCompositor.sdgfxcomp` 注册 `OceanRenderFeature` 到 Water stage / Group1；Editor 侧 `OceanRenderingService` 在 terrain 加载后直接同步 map size/sea level，`Settings.SeaLevel` 同时驱动 ocean plane 与 river `_WaterHeight`。同日 `Terrain.Rendering.CustomForwardRenderer` 已作为 ForwardRenderer replacement 接入 runtime compositor 的 Game/Editor 路径，保留 Stride forward renderer 的 opaque/transparent/depth SRV/opaque SRV/post effects 基础行为，并在 opaque 后、generic transparent 前统一绘制 Ocean 和 River water。

**2026-06-24 Ocean debug1 深水细节响应补充：** 重新复核 `C:\Users\Redwa\Desktop\debug1.rdc` 后，问题不再是深水均值明显偏离 CK3：当前 deep/bottom 区域 final 均值约 `[0.164,0.240,0.265]`，接近 CK3 final 深水 `[0.154,0.218,0.250]`，但空间变化明显不足，当前 G/B 标准差只有约 `0.005..0.013`，CK3 深水约 `0.038`。RenderDoc 热替换验证两条失败路径：单纯把 cubemap reflection 乘大只会让远端掠射水面变亮，前景深水变化仍弱；对深浅水都做统一 reference/detail 对比增强会把 `(980,560)` 这种黑底近岸折射点压到 final `[0.133,0.157,0.153]`。最终 `ApplyOceanDisplayResponse` 改为只对深水使用 detail response：`deepColor = _OceanDisplayDeepBase([0.115,0.185,0.213]) + (finalColor - _OceanDisplayDeepReference([0.065,0.128,0.146])) * _OceanDisplayDeepDetailGain(2.0)`；浅水继续保留 additive bias `finalColor * _OceanDisplayShallowGain(0.8) + _OceanDisplayShallowBias([0.16,0.22,0.20])`，并继续用 `baseRefractionDepth / _OceanDisplayShallowDepth(6.0)` 混合。热替换 gate 中 final EID 997 代表点为 `(1200,820)=[0.141,0.224,0.247]`、`(1400,300)=[0.200,0.255,0.275]`、`(980,560)=[0.235,0.286,0.267]`。该响应仍只存在于 `OceanSurface`，不修改 shared refraction capture、River、global tonemap、scene light，也不引入 province/FOW/flatmap/`_WaterToSunDir`。

**2026-06-24 Ocean CK3-like water color map 路径补充：** `C:\Users\Redwa\Desktop\ck3-ocean远.rdc` 复核确认 CK3 远景海洋不是单一 shallow/deep 颜色：EID 2668 的 shader 直接采样 `WaterColorTexture_Texture(t6)` 作为地图空间区域水色，并绑定 province/border/pattern、`FogOfWarAlpha_Texture`、`FlatMapTexture` 等策略层资源；本帧 `_WaterZoomedInZoomedOutFactor=1.0`、`_WaterRefractionScale=0.0`，因此远景水色主要保留 water color map 路径而不是折射路径。EID 2843/2852 后续还用 `SurroundMask`、`BlackMask`、cloud 等 mask 压暗地图边缘/未探索区域。本项目仍不接入这些策略层。上一版把 `waterColorAndSpec.rgb` 送进 final display response 做保亮度色相转移，导致 `C:\Users\Redwa\Desktop\debug.rdc` 中英伦/北欧坏点 `(790,360)` 的暖棕 `WaterColorTexture` 样本 `[0.1607,0.1045,0.0530]` 被放大成 Ocean 输出 `[0.9006,0.6433,0.3873]`；短暂加入 warm reject 后也被撤掉。当前 `OceanSurface` 改为更接近 CK3 的 pre-display 路径：`ApplyOceanWaterColorMapTint` 用 `_WaterColorMapTint=[0.8324021,0.7931101,1.0]`、`_WaterColorMapTintFactor=0.05` 对 water color map 做轻微 tint；`CalcOceanWaterColor` 在近景只用 facing 在 `DeepColor/ShallowColor` 间取基础水色，不再按 water color alpha 把地图水色直接混进 close diffuse；远景才由 `_OceanZoomedOutWaterColorTextureInfluence=1.0` 让 water map 主导。refraction world-position 重采样继续走同一 tint，并作为 far-zoom fallback。`ApplyOceanDisplayResponse` 不再接收或使用 `waterColorAndSpec.rgb`，只处理当前 water composition 的 deep/shallow display response。shared refraction capture、River、global tonemap/scene light、province/FOW/flatmap/`_WaterToSunDir` 仍不变。

**2026-06-24 Ocean far-map zoom 分支补充：** `C:\Users\Redwa\Desktop\debug.rdc` 再次复核显示全图视角仍过深，且东西方水色差异不明显。RenderDoc 像素 trace 证明当时 Ocean draw 已是 `_WaterColorMapTint` 版本，没有旧 `ApplyOceanRegionalDisplayTint`；开阔水点 raw 只有约 `[0.068,0.081,0.099]`，印度洋/中东附近可低到 `[0.030,0.016,0.032]`。当时的近景 alpha 权重只给地图水色约 `4%`，而本地 `_WaterRefractionScale=500`、CK3 远景捕获为 `0`，远景仍强折射会把深色 refraction/capture 内容拉进海面。`OceanSurface` 因此新增 `ComputeOceanZoomedOutFactor()`，用相机距水面高度在 `_OceanZoomedOutStartHeight=650` 到 `_OceanZoomedOutEndHeight=2500` 之间近似 CK3 `_WaterZoomedInZoomedOutFactor`：远景时 `ComputeOceanWaterColorTextureInfluence` 从近景 `0` 切到 `_OceanZoomedOutWaterColorTextureInfluence=1.0`，`CalcRefraction` 使用 CK3-style `pow(1 - zoom, 2)` 衰减 offset 和 see-through refraction，并以 refraction-space water color map 作为远景 fallback；`ApplyOceanDisplayResponse` 在远景使用 `_OceanZoomedOutDisplayDetailGain=1.0`，避免 deep reference detail gain `2.0` 把低于 reference 的 far-map 水色继续压黑。近景仍保留现有 lighting、reflection、refraction、wave/flow/foam 结构。

**2026-06-24 Ocean far-map response 补充：** 更新后的 `C:\Users\Redwa\Desktop\debug.rdc` 复核确认 far-map zoom 分支已经进入 GPU：Ocean EID 280 的 shader 含 `_OceanZoomedOut*` 路径，cbuffer 中相机高度约 `10108`，`ComputeOceanZoomedOutFactor()` 已饱和到 `1.0`；但最终 EID 939 仍比 CK3 远景偏暗、局部块状深色仍明显。对 `C:\Users\Redwa\Desktop\ck3-ocean远.rdc` 的 EID 2668/3063 对照显示，CK3 Ocean raw 代表点约 `[0.048..0.089,0.071..0.117,0.086..0.133]`，最终 post/LUT 会提升到约 `[0.208..0.310,0.275..0.365,0.310..0.392]`；本项目当前 post 不具备同样的 mapface/LUT 响应。RenderDoc 热替换验证后，`OceanSurface` 在远景新增 `ApplyOceanFarMapResponse`：`farMapColor = _OceanFarMapWaterBase([0.10,0.135,0.15]) + waterColorMap * _OceanFarMapWaterScale([1.0,1.25,1.45])`，只把 `composedColor` 高于 far-map 目标的正向亮部细节按 `_OceanFarMapDetailStrength=0.35` 叠回，避免把 terrain shadow/refraction 的深块重新作为细节带入；`_OceanFarMapResponseStrength=1.0` 让该响应只随 zoom factor 介入远景。该路径仍只在 Ocean shader 内工作，不修改 shared refraction capture、River、global tonemap/scene light，也不引入 province/FOW/flatmap/`_WaterToSunDir`。

**2026-06-24 Ocean close-mapface response 撤回补充：** `C:\Users\Redwa\Desktop\ck3-cocean-ltaly.rdc` 反证了上一版近景 `_OceanCloseMapface*` 全局补偿：CK3 Italy 最终海色明显不是东亚近海的统一青绿。代表纯海水点如 `(420,980)` final 约 `[0.243,0.345,0.361]`，右侧深水 `(1500,760)` final 约 `[0.169,0.153,0.212]`，明显低于 `ck3-ocean-tw.rdc` 多个近海点的 `[0.25,0.46,0.47]` 量级。后续 pixel history 还纠正了早期判断：EID 490 虽绑定水体资源，但这些 Italy 代表点的可见海色主要由更早的 terrain/map-water 路径 EID 385 写入，再经 EID 1263 / final post 进入最终图。因此 `_OceanCloseMapfaceDeepBase=[0.23,0.47,0.48]` / `_OceanCloseMapfaceShallowBias=[0.18,0.33,0.31]` 属于会让 Italy 与 East Asia 近海趋同的全局 trick，已从 `OceanSurface` 和文本测试中撤掉。近景显示响应继续只保留深水 detail / shallow bias 的保守补偿；后续若要收敛东亚近海，应从 CK3 的区域 water-color/depth/refraction/post 链路继续拆分，而不是重新加入全局近景目标色。

**2026-06-24 Ocean close water-color detail 保护补充：** 更新后的 `C:\Users\Redwa\Desktop\debug1.rdc` 证明 close water-color 低频目标能把近景海色推近 `ck3-ocean-tw.rdc`，但上一版 `_OceanCloseWaterColorDetailStrength=0.35` / `_OceanCloseWaterColorResponseStrength=0.85` 会把大面积海面压成均匀青绿色块，丢失浪纹、反射和 refraction 细节。当前 `ApplyOceanWaterColorMapResponse` 保留同一个 map-space water-color 底座、16m shallow mask 和远景 `_OceanFarMap*` 分支，但 close 默认改为 `_OceanCloseWaterColorDetailStrength=1.0`、`_OceanCloseWaterColorResponseStrength=0.70`。在 `zoomedOut=0` 时该组合等价于保留完整 composed water 高频，再叠加 70% 的地图水色偏移；它仍是 Ocean-only 路径，不改 shared capture、River、global tonemap/scene light，也不恢复 `_OceanCloseMapface*` 全局近景常量。

**2026-06-25 Ocean 源路径细节参数补充：** 用户确认当前最终颜色方向已接近 CK3，不再新增 mapface/Tony LUT/ColorCube 或全屏提亮后处理；后续优先提高 Ocean 自身 reflection/fresnel/specular/normal 承载。对 `C:\Users\Redwa\Desktop\debug.rdc` 的 RenderDoc 热替换以 EID 280 / final EID 3445 为 gate，并与 `ck3-cocean-ltaly.rdc` water EID 490 / final EID 1263 对照：`flatten=1.5/1.0/0.8` 与小幅 reflection/specular 增益几乎无效，`flowNormalScale=0.04` 不比原 `0.025` 更好且会增加视觉漂移风险，降低 close response strength 会改变低频颜色。最终采用保守 v6：`_WaterReflectionNormalFlatten=0.6`、`_WaterReflectionIntensity=0.25`、`_WaterSpecular=0.02`、`_WaterGlossScale=0.4`，保持 `_WaterFlowNormalScale=0.025` 和所有 color/response 参数不变。该候选在 mid water 的 final mean 只小幅漂移（约 `+0.001,+0.006,+0.011`），red/blue highpass 与 gradient 上升；v7 虽更强但均值漂移过大，未落地。shader 文本测试同时禁止 Ocean/feature 层引入 `Tony`、`ColorCube`、`Mapface`、`LUT`、`SunIntensity`、`ToneMap` 等 CK3 后处理/亮度 token。

**2026-06-24 River shared capture Task 4 补充：** River 的 runtime/editor 绘制已从 `RiverRenderFeature.Draw` 的私有 `RiverSceneSeed` 路径迁移到 `CustomForwardRenderer.DrawRiverWaterChain`。`GraphicsCompositor.sdgfxcomp` 定义专用 `Water` RenderStage，River selector 指向 Water 而不是 Transparent，因此 Stride 仍负责 collect/culling/sort，但 generic Transparent 不会 double draw River。`CustomForwardRenderer` 在 opaque 后、generic transparent 前扫描 Water stage 的 River ranges，先按 range 汇总 `RiverMaxVisibleCameraHeight` 并在高空相机下跳过 shared capture，再按 range 计算 `_RefractionMaxCameraHeight`，调用 shared `WaterRefractionCapturePass.Capture(...)`，再把 capture texture 传给 `RiverRenderFeature.DrawWaterChain(...)`。`WaterRefractionCapturePass.Capture(...)` 内部恢复调用前的 scene render target，避免污染后续 pass。`RiverRenderResources` 不再分配 `SceneSeedColor`，只维护与 shared capture 同尺寸的 `BottomColor`/`BottomDepth`；bottom pass 前把 shared capture 复制到 `BottomColor`，surface 仍采该工作缓冲。Runtime Game/Editor/SingleView 以及 editor fallback compositor 都使用 `CustomForwardRenderer` 并绑定 Water stage。`RiverRenderFeature` 保留注册用于生命周期/Prepare，旧 `Draw(...)` 明确 no-op；`RiverSceneSeed.sdsl`/key 文件暂保留以避免资产编译 churn，但不再由 `RiverRenderFeature` 创建。
...
**2026-06-21 river Stride lighting 补充：** `debug1.rdc`（无地形）与 `debug2.rdc`（有地形）进一步确认 terrain 的 HDR 输出显著高于 river，是因为 terrain 走 Stride standard material lighting，而 river bottom/surface 仍各自使用较低能量的自定义 lighting。当前已新增 `RiverStrideLighting.sdsl` 共享 mixin，集中实现 Stride-style direct Lambert、GGX direct specular、Schlick Fresnel、Smith-Schlick visibility、polynomial DFG、scene skybox specular、scene skybox diffuse IBL 与共享 scene shadow helper；diffuse IBL 采 scene cubemap 最低频 mip，避免高低地形边界处进入 terrain shadow 的 river bottom 只剩 specular IBL 而近黑。`RiverBottom` / `RiverSurface` 均 mix in 该 helper。`WaterSceneLightingBinder` 现在通过 `RiverStrideLightingKeys` 把同一 scene directional light、shadow cascade、shadow atlas、skybox cubemap、skybox matrix/intensity/mip count 绑定给 water effects；`RiverRenderFeature` 只负责创建 binder 并传入 bottom/surface effects，供后续 ocean render feature 复用同一 scene lighting 绑定路径。Surface 不再声明 `_DefaultEnvironmentSunDiffuse`、`_DefaultEnvironmentSunIntensity`、`_WaterToSunDir`，也不再使用 `ImprovedBlinnPhong`；bottom 不再保留本地 GGX/dominant-reflection IBL。河流仍保持自定义 bottom/refraction/surface pass、refraction、foam、waterFade、reflection 与 depth payload 语义。
...
- `RiverRenderFeature` 负责河底/水面双 pass、与 shared refraction capture 同尺寸的 `BottomColor` 工作缓冲和调试光栅状态；shared capture 由 `CustomForwardRenderer` 的 `WaterRefractionCapturePass` 统一创建
- 河流 shader 统一使用 `TransformationWAndVP` 生成 `PositionWS / PositionH / DepthVS`
- `RiverResourceLoader` 负责从 `game/map/water` 直接加载 bottom diffuse/normal/properties/depth 与 water flowmap/flow normal/foam/ambient/water-color 等 DDS 文件；这些 Bottom/Water 纹理不再通过 Stride `.sdtex`、`Terrain.Editor.sdpkg` `RootAssets` 或 `ContentManager` 管理，缺失本地文件时先写 `Terrain` 日志再让原始文件异常冒泡。`River/Environment/reflection-specular` 仍保持 Stride 内容资源；`RiverRenderFeature` 将这些资源绑定到 `RiverBottom` / `RiverSurface`
...
| `Terrain/Rendering/CustomForwardRenderer.cs` | Runtime Game/Editor compositor 使用的 forward renderer replacement；保留标准 forward 行为，并在 opaque 后、generic transparent 前扫描 Water stage Ocean/River ranges，按 Ocean first、River bottom/surface second 的顺序消费同一次 shared water refraction capture |
| `Terrain/Rendering/Water/WaterRefractionCapturePass.cs` | Renderer-owned shared water refraction capture helper；从 scene color + Presenter depth 写半分辨率 refraction texture，RGB 直通 scene color，alpha 写 camera-relative distance payload，供 Ocean surface 与 River bottom/surface chain 共享 |
| `Terrain/Rendering/Water/WaterRefractionCaptureResources.cs` | Shared water refraction capture RT 生命周期与半分辨率尺寸管理 |
...
| `Terrain/Rendering/River/RiverRenderFeature.cs` | 河流河底/水面双 pass 渲染特性；当前由 CustomForwardRenderer 调用 `DrawWaterChain` 并消费 shared refraction capture，stage `Draw` no-op 防 double draw |
| `Terrain/Rendering/Water/WaterSceneLightingBinder.cs` | 共享 water scene lighting/skybox 绑定；当前供 river bottom/surface 和 ocean surface 使用 |
...
| `Terrain/Rendering/Ocean/OceanResourceLoader.cs` | Ocean water DDS 本地资源加载器；从 `game/map/water` 读取 water color、ambient normal、flowmap、flow normal、foam 等共享水体贴图，不通过 bundle roots；Ocean 按 CK3 resource usage 将 water_color/foam/foam_ramp 作为 sRGB，normal/flow/mask/noise 数据贴图保持 linear |
| `Terrain/Rendering/Ocean/OceanRenderFeature.cs` | Ocean 单 pass 渲染特性；绑定 `OceanSurface`、共享 water DDS、`WaterSceneLightingBinder`、shared refraction capture 和 Ocean material/runtime 参数；runtime/editor compositor selector 指向 Water stage，stage `Draw(...)` no-op，实际由 `CustomForwardRenderer.DrawOceanWater` 调用 |
...
| `Terrain/Effects/Ocean/OceanSurface.sdsl` | Ocean surface pass；采样 CK3-style water DDS，消费 shared refraction capture 的 RGB 与未过滤 alpha payload，实现 core water refraction/fade/see-through/foam/normal/reflection；Ocean `water_color` UV 按本项目 Terrain world-Z 方向直采样，不套用 CK3 源码里的 `1 - v`；flow normal 使用 CK3 FlowMap 方向旋转、`SampleGrad`、`_WaterFlowMapSize * 1.5` 四相位 cubic blend，gradients 按 CK3 源码从旋转前 `NormalCoord` 计算；lighting 复用 `RiverStrideLighting` scene light/skybox |
| `Terrain/Effects/Ocean/OceanVertexStreams.sdsl` | Ocean quad 自定义 UV stream |
| `Terrain/Effects/Water/WaterRefractionCapture.sdsl` | Shared water refraction capture image shader；RGB 直通 scene color，并在 alpha 写 RiverCommon camera-relative world-space distance payload |
