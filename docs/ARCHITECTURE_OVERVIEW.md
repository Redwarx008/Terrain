...
| **材质系统** | ✅ 已实现 | - |
| **map TOML 规格** | ✅ 已记录 | [map-data-toml-formats](design/map-data-toml-formats.md)；2026-06-21 CK3 runtime cbuffer 对照确认 terrain `HeightScale=50.0`、`MaxHeight=50` refraction camera clamp 是两个不同常量。本项目默认 `height_scale=200` 可保留，河流 refraction distance payload 通过 `_RefractionMaxCameraHeight=max(50, HeightScale)` 支持高地形高度，避免把 CK3 的固定 clamp 当成全局地形高度限制。 |
| **虚拟纹理** | 🚧 进行中 | - |
...
**2026-06-19/21 surface alpha 与 water-color 导入补充：** 更新后的 `debug.rdc` 证明 foam 白块已消失，但 bank-edge 仍被 surface 以 alpha `1.0` 写入暗 see-through 色。早期曾按 `jomini_river_surface.fxh` 里的 `CalcRiverSurface` 误判为 depth alpha：`saturate(Depth * 2.0 / _Depth) * Transparency * connectionFade`；2026-06-21 复查 CK3 `river_surface.shader` 后确认实际入口是 `CalcRiverAdvanced(Input)._Color`，其中 `Depth = CalcDepth(Input.UV)` 只用于 `Params._Depth = Depth * Input.Width + 0.1f` 并传入 `CalcWater` / `WaterFade` / refraction，最终 alpha 在 refraction 分支下是 `Transparency * saturate((DistanceToMain - 0.1f) * 5.0f) * smoothstep(0, _BankFade, UV.y) * smoothstep(0, _BankFade, 1 - UV.y)`。对 `C:\Users\Redwa\Desktop\debug.rdc` 的 surface draw `309` 做热替换诊断后，坏点 `(500,204..219)` 的 advanced alpha 仍为 `1.0`，代表点 `(500,204)` 的 `RiverUV.y=0.1321603`、`DistanceToMain=1`、`Transparency=1`，已经远超 `_BankFade=0.025` 的过渡带；因此“扩大 surface alpha 岸边过渡”不是 CK3 parity 修复。继续热替换证明 raw `RefractionTexture` 在同点仍有棕色 bottom `[0.635,0.409,0.204]`，但 `CalcTerrainUnderwaterSeeThrough` 的 attenuation 只有 `0.02~0.03`、`refractionDepth≈6~8`，把 bottom 几乎全部 lerp 到 water-color map；曾尝试将 see-through 深度限制为 `min(refractionDepth, Depth)`，可让 bottom 暂时显现，但会让全水面过度保留 bottom/refraction、丢失 CK3 水体颜色。再次热替换只调 `_WaterSeeThroughDensity` 时，`0.6` 变化很小，`0.4` 能明显增加 bottom 可见度且仍保留水色，`0.2` 已偏 bottom-heavy；因此后续源码落地应走显式 density/距离尺度参数化，而不是改变 CK3 `RefractionDepth` 调用结构。CK3 源码确认真正控制链是：`CalcTerrainUnderwaterSeeThrough(RefractionDepth, RefractionWorldSpacePos, RefractionWaterColorMap, RefractionSample.rgb)` 使用 `_WaterSeeThroughDensity` 和 `Depth / ToCameraDir.y` 控制透底；`WaterFade` 另用 `min(Input._Depth, RefractionDepth)` 控制 lit water color 强度；最终 `FinalColor += lerp(Refraction, Reflection, Fresnel * WaterFade)`。当前 `RiverSurface` 已移除上一轮 `_SurfaceBankFade` workaround，恢复 `CalcRiverAdvanced` 的 `_BankFade` edge alpha，同时保持 CK3 的 `RefractionDepth` see-through 路径。bottom/water DDS 统一绕过 Stride 内容系统，从 `game/map/water` 直接 `Texture.Load`；`water_color.dds` 当前使用 `loadAsSrgb:false` 创建 UNorm/linear GPU texture view，`RiverSurface` 不再手动 `DecodeWaterColorSrgb`。
...
- Runtime 不在 `TerrainComponent` 上常驻完整 `ushort[]` heightmap。`TerrainComponent` 只暴露离散 `GetHeight(int sampleX, int sampleZ)`；内部经 `TerrainHeightSampler` 从 `.terrain` 按需同步读取对应 height tile，并只保留 4 个 CPU height tile 缓存。Runtime DetailMap 构建和 River mesh 生成都复用 `TerrainComponent.GetHeight`；River 需要连续高度时自己对 4 个离散 sample 做双线性插值，Terrain 不承担 river-specific 插值策略。
...
| `Terrain/Streaming/TerrainHeightSampler.cs` | CPU 侧离散 height sample 按需 tile 读取与 4-tile page cache |
