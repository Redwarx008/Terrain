...
**2026-06-19/20 surface 后段资源/参数链补充：** CK3 surface 在 `CalcRiverAdvanced -> CalcWater` 后还会执行 cloud shadow、terrain shadow tint、strategy fog-of-war 与 map distance fog；本项目曾分阶段接入这些 wrapper 后段，并删除了 strategy-layer FOW 依赖。2026-06-20 `debug3.rdc` / `debug4.rdc` 证明 procedural cloud shadow 的 `_GlobalTime` 相位会让同一 river surface 在 capture 对比中从亮色跳到暗色；因此曾短暂把 `cloudMask` 固定为 0，只保留确定性 terrain shadow tint 和 map distance fog。用户要求完全恢复到 `debug4.rdc` 后，当前 `RiverSurface` 已重新恢复完整 debug4-style `ApplySurfacePostProcessing` 后段：`GetCloudShadowMask`、`_HasCloudShadowEnabled`、`_InverseWorldSize`、`_GlobalTime * 0.01f` 云影移动、`cloudMask * 0.8f` 水色暗化、`_MapSadowTintNoiseUVTiling=500`、`_MapSadowTintStrength=0.5`、`_MapSadowTintThresholdMin/Max=0.1/0.4`、`_TerrainSunnySunDir`、`ApplyOvercastContrast`、`_FogBegin2/_FogEnd2/_FogMax`、`_FogColor/_RelativeFogColor`、relative height fog 与 noise modulation。`RiverRenderFeature` 会绑定 `shadow_color.dds`、`TerrainHeightSampler`、editor height slices、terrain bounds、`_InverseWorldSize` 与 `_HasCloudShadowEnabled`；terrain 不可用时 `SliceCount=0` 走 up-normal fallback，不跳过 surface draw。策略层 FOW 仍删除。`debug5.rdc` 的 `WaterColorTexture.rgb * 8` 热替换只证明 wrapper 删除后 raw water body 会偏黑；源码仍不保留 `_WaterColorSurfaceLift` 与 `visibleWaterColor` floor。注意：完全恢复 debug4 也意味着多次切换地形显隐后截图时，如果 `_GlobalTime` 相位不同，河流颜色仍可能不同。
